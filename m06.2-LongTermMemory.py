...
#   -> Add short-term memory as a part of your agent's state to enable multi-turn conversations. (e.g. Checkpointers)
#   -> Add long-term memory to store user-specific or application-level data across sessions. (e.g. Facts{Semantic Memories}, Experiences{Episodic Memories}, Rules{Procedural Memories})
...
def save_recall_memory(memory: str, config: RunnableConfig) -> str:
    ...
    if memory not in store.get()['documents']:
...
# These Memory Store functions dont need to be tools, but it is more efficient to do so
...
# When using long-term memory, it is important to provide a user_id to identify different users
# You can also provide a thread_id to separate different conversations for the same user
# Beware of thread_ids as they belong to a specific user_id, otherwise a thread will go to a different user without the Long-Term Memory
...
# Here we created a different thread for the same user
...
# This is a mistake, as this thread_id belongs to a different user. Therefore, the agent will not have ang LTM but will still have the STM of the current conversation
...
# Here we created a different user
